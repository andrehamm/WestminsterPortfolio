    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        closestPosition = DiscreteDistribution()
        for ghost in livingGhostPositionDistributions:
            closestPosition[ghost.argMax()] = self.distancer.getDistance(pacmanPosition, ghost.argMax())
            
        closest = min(closestPosition, key=closestPosition.get)

        bestAction = DiscreteDistribution()
        for action in legal:
            newPos = Actions.getSuccessor(pacmanPosition, action)
            bestAction[action] = self.distancer.getDistance(newPos, closest)

        action = min(bestAction, key=bestAction.get)
        return action