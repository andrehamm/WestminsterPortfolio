...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        total = 0
        for key in copy:
            if pick >= total and pick <= (total+copy[key]):
                return key
            total += copy[key]
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        if ghostPosition == jailPosition and noisyDistance is None:
            ...
            return 0
        elif not noisyDistance is None and ghostPosition == jailPosition:
            return 0
        else:
    ...
    def elapseTime(self, gameState):
        ...
        for oldPosition in self.allPositions:
            ...
            for newPosition in newPosDist:
                tempDict[newPosition] = tempDict[newPosition] + self.beliefs[oldPosition]*newPosDist[newPosition]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        if dist.total() == 0:
            ...
            for particle in range(self.numParticles):
                newParticles.append(DiscreteDistribution.sample(dist))
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        newParticles = []
        particleDict = DiscreteDistribution()
        for particle in self.particles:
            if not particle in particleDict:
                newPosDist = self.getPositionDistribution(gameState, particle)
                particleDict[particle] = newPosDist
            newParticles.append(DiscreteDistribution.sample(particleDict[particle]))
        self.particles = newParticles
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        numPartPer = self.numParticles/ len(legalPos)

        for position in self.legalPositions:
            for partper in range(int(numPartPer)):
                self.particles += legalPos
        if len(legalPos) > len(self.particles):
            self.particles += legalPos
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ghostProb = self.getObservationProb(observation[i], gameState.getPacmanPosition(), particle[i], self.getJailPosition(i))
                temp *= ghostProb
            dist[particle] += temp
        if dist.total() == 0:
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                newParticle[i] = DiscreteDistribution.sample(newPosDist)
            """*** END YOUR CODE HERE ***"""
