...
public class WaterGUI extends JFrame {
	...
	private JButton accountInfo = new JButton("View Account Info");
	private JButton allInvoices = new JButton("All Invoices by Account");
	private JButton fiscalYear = new JButton("Overall Usage and Spending by Fiscal Year");
	private JButton month = new JButton("Overall Usage and Spending by Month");
	private JButton accountYear = new JButton("Compare Annual Usage and Spending for Select Account");
	private JButton accountMonth = new JButton("Compare Usage and Spending by Account by Month");
	...
	private void setupLayout() {
		...
		month.setFont(font);
		accountYear.setFont(font);
		accountMonth.setFont(font);
		...
		main.add(fiscalYear);
		main.add(month);
		main.add(accountYear);
		main.add(accountMonth);
		main.add(delete);
	...
	private void addListeners() {
		...
		//If the usage by month button is clicked, the addAccount method is called.
		...
		accountYear.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				queryAccountYear();
			}
		...
		accountMonth.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				queryAccountMonth();
			}
	...
	private void addInvoice() {
		...
		//JTextField serviceFrom = new JTextField();
		//JTextField serviceTo = new JTextField();
		...
		String[] asString = {"Invoice Number: (ex. 12345678910) ", "Account Number: (ex. 1234567)",
				...
				"Adjustments: (ex. XXX.XX)", "Total Charges: (ex. XXX.XX)"};
		...
		invoiceOK.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String dueDateInput = dueDate.toString();
				String serviceFromInput = serviceFrom.toString();
				String serviceToInput = serviceTo.toString();

				boolean success = true;
				//Connects to database and enters user input.
	...
	private void queryFiscalYear() {
		...
		Object[] message = {"Compare fiscal years since: ", year};
		int option = JOptionPane.showConfirmDialog(null, message, "Query Fiscal Year", JOptionPane.OK_CANCEL_OPTION);
		if (option == JOptionPane.OK_OPTION) {

			String yearString = year.toString();
			int start = Integer.parseInt(yearString);
			int end = currentYear;

			if (start <= currentYear) {
				//try connection
				//perform query (append multiple queries)
				//show table
				try (Connection conn = DriverManager.getConnection(DB_URL)) {
					Statement s = conn.createStatement();
					ResultSet results = null;
					String[] queries = query.waterUsedInFiscalYear(start, end);
					for (int i = 0; i < queries.length; i++) {
						results = s.executeQuery(queries[i]);
						//take each query and append to the table within the loop
					}
					while (results.next()) {
						int id = results.getInt(1);
						String sizeString = results.getString(2);
						double price = results.getDouble(3);
						//Pizza.Size size = sizeFromString(sizeString);

						//Pizza p = new Pizza(id, size, price);
						//tableModel.addInstance(p);
					}
				} catch (SQLException ex) {
					ex.printStackTrace();
					JOptionPane.showMessageDialog(this,
							"Unable to download pizzas from database. :-(",
							"Bad database!",
	...
	private void queryMonth() {
		...
		JSpinner.DateEditor dateFormat = new JSpinner.DateEditor(year, "yyyy");
		year.setEditor(dateFormat);
		dateFormat = new JSpinner.DateEditor(month, "MM");
		month.setEditor(dateFormat);

		Object[] message = {"Compare the following month: ", month, "For fiscal years beginning: ", year};
		int option = JOptionPane.showConfirmDialog(null, message, "Monthly Usage and Spending", JOptionPane.OK_CANCEL_OPTION);
		if (option == JOptionPane.OK_OPTION) {

			String startString = year.toString();
			int start = Integer.parseInt(startString);
			int end = currentYear;
			String monthString = month.toString();
			int thisMonth = Integer.parseInt(monthString);
	...
	private void queryAccountYear() {
		...
		Object[] message = {"Compare fiscal years since: ", year, "For the account: ", account};
		int option = JOptionPane.showConfirmDialog(null, message, "Monthly Usage and Spending", JOptionPane.OK_CANCEL_OPTION);
		if (option == JOptionPane.OK_OPTION) {
			...
			int start = Integer.parseInt(startString);
			int end = currentYear;
			int accountInt = Integer.parseInt(accountString);
	...
	/**
	 * Queries fiscal years for a select month for a single account, user inputs accountNum, month and starting year. 
	 */
	private void queryAccountMonth() {
		...
		Object[] message = {"Compare the following month: ", month, "For fiscal years beginning: ", year, "For the account: ", account};
		int option = JOptionPane.showConfirmDialog(null, message, "Monthly Usage and Spending", JOptionPane.OK_CANCEL_OPTION);
		if (option == JOptionPane.OK_OPTION) {
	...
	private void deleteInv() {
		...
		Object[] message = {"Delete the following invoice: ", invoice};
		...
		if (option == JOptionPane.OK_OPTION) {
			...
			int invoiceInt = Integer.parseInt(invoiceString);
			//try connection
			//perform query
			//show table
			//query.
	...
	private void accountInfo() {
		JTextField account = new JTextField();
		JLabel info = new JLabel("Show all information for the account: ");
		info.setFont(font);

		Object[] message = {info, account};
		int option = JOptionPane.showConfirmDialog(null, message, "Account Information", JOptionPane.OK_CANCEL_OPTION);
		//if ok button is clicked database is queried
		if (option == JOptionPane.OK_OPTION) {

			String accountString = account.getText();
			accountTable(accountString);
			//try connection
			//perform query
			//show table
			//query.
	...
	private void accountTable(String accountString) {
		//use JDialog instead
		JFrame accountQuery = new JFrame();
		setLayout(new BorderLayout(5, 5));
		accountTableModel = new AccountTableModel();
		List<Account> accounts = fetchAccounts(accountString);
	...
	private List<Account> fetchAccounts(String accountString) {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL)) {
			...
			// while (results.next()) {
			// 	System.out.println(results.getInt("accountNum"));
			// }
